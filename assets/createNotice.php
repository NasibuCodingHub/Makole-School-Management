...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($fileSize > $allowedFileSize) {
        ...
        $query = "INSERT INTO `notice` (`s_no`, `sender_id`, `editor_id`, `title`, `body`, `importance`, `file`, `timestamp`, `role`, `class`) VALUES (NULL, ?, ?, ?, ?, ?, ?, current_timestamp(), ?, ?)";
        $stmt = mysqli_prepare($conn, $query);
        $role = '';  // Set role value
        $class = ''; // Set class value
        mysqli_stmt_bind_param($stmt, "ssssssss", $senderId, $senderId, $title, $body, $importance, $file, $role, $class);
