...
if (mysqli_num_rows($result) > 0) {
    echo 'Email already exists for this user type!';
}
else{
    ...
    if (!$invalidFormat) {
        ...
        // Insert parent details into users table
        $addUserDetailQuery = "INSERT INTO `users` (`s_no`, `id`, `email`, `password_hash`, `role`, `theme`) VALUES (NULL, ?, ?, ?, 'parent', 'light')";
       $parentEmail="parent.".$email;
        $stmt = mysqli_prepare($conn, $addUserDetailQuery);
        mysqli_stmt_bind_param($stmt, "sss", $uniqueId, $parentEmail, $passwordHash);
        mysqli_stmt_execute($stmt);
        ...
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            $response = 'Error - Unable to add student and parent details';
        }
    }}
} else {
