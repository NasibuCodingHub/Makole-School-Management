...
if (mysqli_num_rows($result) > 0) {
    ...
    if ($row2["request"] == "") {
        echo '<button type="button" data-uid="' . htmlspecialchars($uid) . '" id="request" class="btn btn-primary" data-mdb-ripple-init>
        <i class="fas fa-paper-plane me-2"></i> Request For Bus
      </button>';
      
        $sql = "UPDATE students SET request='pending', request_date=CURRENT_DATE, request_time=CURRENT_TIME WHERE id=?";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <a href='buslocation.php?bus_id=" . htmlspecialchars($row['bus_id']) . "' class='text-decoration-none text-dark'>
                        <div class='d-flex align-items-center'>
                            <div class='bus-icon'></div>
                            <div class='ms-3'>
                                <h5 style='font-size: 15px; padding: 10px;'>Bus No: " . htmlspecialchars($row['bus_number']) . " <br>Title: " . htmlspecialchars($row['bus_title']) . "</h5>
                            </div>
                        </div>
                      </a>
                      </div>";
